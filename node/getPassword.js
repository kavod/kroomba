...
const discovery = require('dorita980/lib/discovery.js');

if (!process.argv[3]) {
  console.log('Use: npm run getpassword <robot_ip_address> <blid> [firmware version]');
  process.exit();
}

const host = process.argv[2];
const blid = process.argv[3];
const fversion = process.argv[4];
...
function checkV2 () {
  ...
  client.on('data', function (data) {
    ...
    if (data.length <= 7) {
      ...
      console.log('Password ' + new Buffer(data).slice(sliceFrom).toString());
      console.log('Use this credentials in dorita980 lib :)');
    }
    client.end();
    process.exit(0);
